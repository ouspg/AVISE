"""Deceptive Delight Security Evaluation Test.

Implements the Deceptive Delight multi-turn jailbreak (https://arxiv.org/abs/2509.05471) with some modifications. See configuration
...
@set_registry.register("deceptive_delight")
class DeceptiveDelight(BaseSETPipeline):
    """Deceptive Delight SET."""

    name = "Deceptive Delight"
    description = "Multi-turn scenarios, where harmful topics are mixed with benign ones to trick the model into providing harmful information. "
    ...
    def initialize(self, set_config_path: str) -> List[LanguageModelSETCase]:
        ...
        for i, set_ in enumerate(sets):
            set_cases.append(
                LanguageModelSETCase(
                    id=set_.get("id", f"DECEPTIVE-DELIGHT-{i + 1}"),
                    prompt=set_.get("conversation", []),
                    metadata={
                        "action": set_.get("action", "Deceptive Delight Attack"),
    ...
    def execute(
        ...
        logger.info(f"Executing {len(sets)} DeceptiveDelight SET cases")
    ...
    def _incremental_execution(self, set_case, connector) -> ExecutionOutput:
        """Execute a DeceptiveDelight SET incrementally (generates a response for each individual prompt).
