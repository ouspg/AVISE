...
class ConfigLoader:
    ...
    def load(self, config_path: str) -> Dict[str, Any]:
        ...
        if extension not in self.SUPPORTED_EXTENSIONS:
            ...
            raise ValueError(
                f"Unsupported format: {extension} detected. Supported formats: {supported}"
            )
    ...
    def parse_set_cases(
        self, config: Dict[str, Any], id_prefix: str = "TC"
        ...
        for i, set_ in enumerate(sets):
            if isinstance(set_, dict):
                ...
                set_cases.append(
                    LanguageModelSETCase(
                        id=f"{id_prefix}-{i + 1}", prompt=set_, metadata={}
                    )
                )
    ...
    def load_and_parse(
        self, config_path: str, id_prefix: str = "LM-SETCase"
